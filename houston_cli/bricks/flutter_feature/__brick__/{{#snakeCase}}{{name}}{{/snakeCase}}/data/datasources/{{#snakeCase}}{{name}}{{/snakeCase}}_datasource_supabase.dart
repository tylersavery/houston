...
class {{#pascalCase}}{{name}}{{/pascalCase}}DataSourceSupabaseImpl implements {{#pascalCase}}{{name}}{{/pascalCase}}DataSource {
  ...
  Future<{{#pascalCase}}{{name}}{{/pascalCase}}> retrieve(int id) async {
    try {
      final result = await client.from("{{#snakeCase}}{{name}}{{/snakeCase}}").select("*").eq('id', id).single();
  ...
  Future<{{#pascalCase}}{{name}}{{/pascalCase}}> save({{#pascalCase}}{{name}}{{/pascalCase}} {{#camelCase}}{{name}}{{/camelCase}}) async {
    try {
      if ({{#camelCase}}{{name}}{{/camelCase}}.id == null) {
        final result = await client.from("{{#snakeCase}}{{name}}{{/snakeCase}}").insert({{#camelCase}}{{name}}{{/camelCase}}.toJson()).select("*").single();
        return {{#pascalCase}}{{name}}{{/pascalCase}}.fromJson(result);
      } else {
        final result = await client.from("{{#snakeCase}}{{name}}{{/snakeCase}}").update({{#camelCase}}{{name}}{{/camelCase}}.toJson()).match({"id": {{#camelCase}}{{name}}{{/camelCase}}.id}).select("*").single();
        return {{#pascalCase}}{{name}}{{/pascalCase}}.fromJson(result);
      }
    } catch (e) {
      throw ServerException(e.toString());
