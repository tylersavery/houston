...
class SupabaseBlueprintSerializer extends BlueprintSerializer {
  ...
  String get generatedSql {
    ...
    for (final property in properties) {
      ...
      if (propertyName == 'uid') {
        if (name == "profile") {
          ...
          sqlProperties
              .add('"uid" "uuid" DEFAULT "gen_random_uuid"() NOT NULL');
        }
      } else if (type == "char") {
        sqlProperties.add(
            '"$propertyName" character varying${!property.allowNull ? " NOT NULL" : ""}${defaultValue != null ? ' DEFAULT $defaultValue' : ''}');
      } else if (type == "text" || type == "string" || type == "url") {
        sqlProperties.add(
            '"$propertyName" "text"${!property.allowNull ? " NOT NULL" : ""}${defaultValue != null ? ' DEFAULT $defaultValue' : ''}');
      } else if (type == "int" || type == "double") {
        sqlProperties.add(
            '"$propertyName" numeric${!property.allowNull ? " NOT NULL" : ""}${defaultValue != null ? ' DEFAULT $defaultValue' : ''}');
      } else if (type == "datetime") {
        if (propertyName == "created_at") {
          sqlProperties.add(
              '"$propertyName" timestamp with time zone DEFAULT "now"() NOT NULL');
        } else {
          sqlProperties.add(
              '"$propertyName" timestamp with time zone${!property.allowNull ? " NOT NULL" : ""}');
        }
      } else {
        sqlProperties.add(
            '"$propertyName" bigint${!property.allowNull ? " NOT NULL" : ""}');
      }
    }
    ...
    sql.add(
        'ALTER TABLE ONLY "public"."$name" ADD CONSTRAINT "${name}_pkey" PRIMARY KEY ("id");');
