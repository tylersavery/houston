...
class GameSystemDataSourceSupabaseImpl implements GameSystemDataSource {
  ...
  Future<GameSystem> retrieve(int id) async {
    try {
      final result = await client.from("game_system").select("*").eq('id', id).single();
  ...
  Future<GameSystem> save(GameSystem gameSystem) async {
    try {
      if (gameSystem.id == null) {
        final result = await client.from("game_system").insert(gameSystem.toJson()).select("*").single();
        return GameSystem.fromJson(result);
      } else {
        final result = await client.from("game_system").update(gameSystem.toJson()).match({"id": gameSystem.id}).select("*").single();
        return GameSystem.fromJson(result);
      }
    } catch (e) {
      throw ServerException(e.toString());
