...
class ProfileDataSourceSupabaseImpl implements ProfileDataSource {
  ...
  Future<PaginatedResponse<Profile>> list({required int page, required int limit}) async {
    try {
      ...
      return PaginatedResponse<Profile>(
        results: result.data.map<Profile>((item) => Profile.fromJson(item)).toList(),
        status: 200,
        count: result.count,
        page: page,
        limit: limit,
  ...
  Future<Profile> retrieve(int id) async {
    try {
      final result = await client.from("profile").select(defaultSelect).eq('id', id).single();
  ...
  Future<Profile> save(Profile profile) async {
    try {
      if (profile.id == null) {
        final result = await client.from("profile").insert(profile.toJson()).select(defaultSelect).single();
        return Profile.fromJson(result);
      } else {
        final result = await client.from("profile").update(profile.toJson()).match({"id": profile.id}).select(defaultSelect).single();
        return Profile.fromJson(result);
      }
    } catch (e) {
      throw ServerException(e.toString());
